...
private enum ShareExtensionError: LocalizedError {
    case missingDataRepresentation(String)

    var errorDescription: String? {
        switch self {
        case .missingDataRepresentation(let type):
            "No data representation was returned for shared item type \(type)."
        }
    }
}
    ...
    func loadDataRepresentation(for type: UTType) async throws -> Data {
        try await withCheckedThrowingContinuation { cont in
            loadDataRepresentation(forTypeIdentifier: type.identifier) { data, err in
                ...
                else { cont.resume(throwing: ShareExtensionError.missingDataRepresentation(type.identifier)) }
            }
        }
    }
}
